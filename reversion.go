...
//	fmt.Print("the number of goroutine: ")
...
//	RTemp := mkPS2()
	...
	go func() {	
		put(zero, R)
		InvF1 := i2tor(F1.den, F1.num)
//		print("F1: ")
//		print(F1.num)
//		print(" / ")
//		print(F1.den)
//		print("  "	)	
		NegInvF1 := i2tor(-F1.den, F1.num)
		split(R_[0], R_0)
		//copy(Cmul(NegInvF1, Xmul(Mul(Mul(R_2[0], R_2[1]), Subst(F, R_1[0])))), R)
		split(Shift(InvF1, Cmul(NegInvF1, Mul(Square(R_0[1]), Subst(F, Shift(zero, R_0[0]))))), R_)
		copy(R_[1], R)	
	}()
	return R
}
...
func Init() {
  ...
  func main() {
	...
	print("Exp: ")
	//Printn(Exp(Ones), 15)
//	Printn(Recip(Mul(Exp(Ones1), Ones2)), 10)	
	tan := Revertion(Integ(zero, MonSubst(Ones, neg(one), 2)))
	Printn(tan, 15)
//Printn(Mul(Ones1, Ones2), 100)  	
//  		print("Ones: ")
//  		Printn(Ones, 10)
//  		print("Twos: ")
//  		Printn(Twos, 10)
